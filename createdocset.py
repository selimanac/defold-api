...
def get_ref_doc():
    ...
    urllib.request.urlretrieve(
        "http://d.defold.com/archive/" + sha1 + "/engine/share/ref-doc.zip", DOC_ZIP)
...
def create_docset():
            ...
            for file in files:
                with open(os.path.join(root, file), "r") as fh:
                    if file.endswith(".json"):
                        ...
                        class_path = class_name + ".md"
                        class_doc = ""
                        parsed_json = json.load(fh)
                        info = parsed_json["info"]
                        class_doc = class_doc + "<h1>" + \
                            info["name"] + "</h1>\n"
                        class_doc = class_doc + "<p>" + \
                            info["description"] + "</p>\n"
                        index_html = index_html + "<a class='index' href='ref/" + \
                        ...
                        for element in parsed_json["elements"]:
                            ...
                            if function_name != "":
                                ...
                                class_doc = class_doc + "\n#" + function_name +  ("()" if entry_type == "Function" else "") + "\n"
                                class_doc = class_doc + "<div class='brief'>" + \
                                    element["brief"] + "</div>"
                                if element.get("description", "") != "":
                                    class_doc = class_doc + "<p>" + \
                                        element["description"] + "</p>"
                                if element.get("note", "") != "":
                                    class_doc = class_doc + "<p>Note: " + \
                                        element["note"] + "</p>"
                                if len(element["parameters"]) > 0:
                                    class_doc = class_doc + "<h3>PARAMETERS</h3>"
                                    class_doc = class_doc + "<div class='params'>"
                                    for parameter in element["parameters"]:
                                        class_doc = class_doc + "<p class='param'>" + \
                                            parameter["name"] + " - " + \
                                                parameter["doc"] + "</p>"
    ...
    with open(os.path.join(documents_path, "index.md"), "w") as out:
